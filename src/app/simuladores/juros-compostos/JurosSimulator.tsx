    ...
    <div className="space-y-4">
      <div className="card">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Capital inicial (€)</label>
            <input type="number" value={capital} onChange={(e) => setCapital(e.target.value)} className="input-field" placeholder="10000" min="0" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Aporte mensal (€)</label>
            <input type="number" value={aporte} onChange={(e) => setAporte(e.target.value)} className="input-field" placeholder="200" min="0" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Taxa anual (%)</label>
      ...
      {result && (
        <div className="card animate-fade-in space-y-4">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">Resultado ao fim de {anos} anos</h3>
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 text-center">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Capital final</p>
              <p className="font-bold text-[#046A38] dark:text-green-400 text-lg">{fmt(result.total)}</p>
            </div>
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 text-center">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Total investido</p>
              <p className="font-bold text-blue-700 dark:text-blue-400 text-lg">{fmt(result.invested)}</p>
            </div>
            <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4 text-center">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Juros ganhos</p>
              <p className="font-bold text-yellow-700 dark:text-yellow-400 text-lg">{fmt(result.gain)}</p>
            </div>
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-gray-100 dark:border-gray-700">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Ano</th>
                  <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Valor acumulado</th>
                  <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">% crescimento</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
                {result.byYear.filter((_, i) => i % Math.max(1, Math.floor(result.byYear.length / 10)) === 0 || i === result.byYear.length - 1).map((row) => (
                  <tr key={row.year} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-4 py-2.5 font-medium text-gray-900 dark:text-gray-100">{row.year}</td>
                    <td className="px-4 py-2.5 text-right font-medium text-[#046A38] dark:text-green-400">{fmt(row.value)}</td>
