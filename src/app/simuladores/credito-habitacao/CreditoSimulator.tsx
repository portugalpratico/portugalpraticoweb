    ...
    <div className="space-y-4">
      <div className="card">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Capital (€)</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Euribor (%)</label>
            <input type="number" value={euribor} onChange={(e) => setEuribor(e.target.value)} className="input-field" placeholder="3.0" step="0.1" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Spread (%)</label>
      ...
      {result && (
        <div className="card animate-fade-in space-y-4">
          <div className="bg-green-50 border border-green-100 rounded-xl p-5 text-center">
            <p className="text-sm text-gray-500 mb-1">Prestação Mensal</p>
            <p className="text-4xl font-bold text-[#046A38]">{fmt(result.prestacao)}</p>
            <p className="text-xs text-gray-400 mt-1">Taxa total: {result.taxa.toFixed(2)}% ao ano</p>
          </div>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              <p className="text-xs text-gray-500 mb-1">Total pago</p>
              <p className="font-bold text-gray-900">{fmt(result.totalPago)}</p>
            </div>
            <div className="bg-red-50 rounded-xl p-3 text-center">
              <p className="text-xs text-gray-500 mb-1">Total em juros</p>
              <p className="font-bold text-red-600">{fmt(result.totalJuros)}</p>
            </div>
          </div>
          <p className="text-xs text-gray-400">* Simulação indicativa. Não inclui seguros, comissões ou outros encargos. Consulte o seu banco.</p>
        </div>
      )}
    </div>
  );
